...
vultr_provision.py
==================
Alternative to Terraform: a standalone Python script that uses the Vultr
REST API v2 to provision all ArdouraAI infrastructure components.
...
CONFIG = {
    ...
    "db_engine": "mysql",
    "db_version": "8",
    "db_plan": "vultr-dbaas-startup-cc-1-55-1",  # smallest MySQL tier
    "state_file": Path(__file__).parent / ".vultr_state.json",
}


# ── Helpers ─────────────────────────────────────────────────────────────────
def v(path: str, method="GET", body: dict | None = None) -> dict:
    """Thin wrapper around the Vultr REST API."""
    if not API_KEY:
...
def wait_for(resource_type: str, resource_id: str, field: str, target: str,
    ...
    print(f"  Waiting for {resource_type}/{resource_id} {field}={target} ...", end="", flush=True)
    while time.time() < deadline:
        data = v(f"{resource_type}/{resource_id}")
        # Vultr wraps response in the singular resource name
        ...
        time.sleep(poll)
    raise TimeoutError(f"{resource_type}/{resource_id} did not reach {field}={target} within {timeout}s")


# ── Startup script (cloud-init) ──────────────────────────────────────────────
def build_startup_script_payload() -> str:
    """Base64-encode the startup.sh and wrap it in a Vultr startup-script POST."""
...
def create_database(state: dict) -> dict:
    ...
    db = r["database"]
    state["db_id"] = db["id"]
    state["db_host"] = db.get("host", "")
    state["db_port"] = db.get("port", 3306)
    state["db_user"] = db.get("user", "")
    state["db_password"] = db.get("password", "")
    save_state(state)
    print(f"  DB cluster provisioning: {state['db_id']}")
    wait_for("databases", state["db_id"], "status", "Running", timeout=900)
    # Refresh credentials after Running
...
def create_instance(state: dict) -> dict:
    ...
    state["instance_ip"] = inst.get("main_ip", "")
    save_state(state)
...
def action_create() -> None:
    ...
    print("=" * 60)
    print(f"  App IP:       {state.get('instance_ip')}")
    print(f"  Webhook URL:  http://{state.get('instance_ip')}:5000/jira-webhook")
    print(f"  Health:       http://{state.get('instance_ip')}:5000/health")
    print(f"  MySQL host:   {state.get('db_host')}:{state.get('db_port')}")
    print("")
    print("Next steps:")
    print(f"  1. SSH:    ssh root@{state.get('instance_ip')}")
    print("  2. Fill:   nano /etc/ardoura/secrets.env")
    print("  3. Reload: systemctl restart ardoura")
    print("  4. Verify: curl http://" + state.get('instance_ip', 'IP') + ":5000/health")
...
if __name__ == "__main__":
    ...
    actions = {"create": action_create, "status": action_status, "destroy": action_destroy}
    actions[args.action]()
