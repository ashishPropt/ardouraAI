...
print(f"[Producer] Kafka bootstrap: {BOOTSTRAP_SERVERS}")
...
def jira_webhook():
    ...
    if project_key != TARGET_PROJECT_KEY:
        return jsonify({'status': 'ignored', 'reason': f'project is {project_key}'}), 200
...
def dynatrace_webhook():
    ...
    alert = {
        "source": "dynatrace", "alert_id": pid,
        "title": data.get('ProblemTitle', data.get('title', 'Dynatrace Alert')),
        "severity": data.get('ProblemSeverity', data.get('severity', 'UNKNOWN')),
        "status": state,
        "impact": data.get('ProblemImpact', data.get('impact', '')),
        "tags": data.get('Tags', data.get('tags', [])),
        "affected": data.get('ImpactedEntities', data.get('affectedEntities', [])),
        "problem_url": data.get('ProblemURL', data.get('problemUrl', '')),
        "timestamp": int(time.time()), "raw": data
    }
    producer.send('dynatrace-alerts', alert)
    producer.flush()
...
def datadog_webhook():
    ...
    alert = {
        "source": "datadog", "alert_id": alert_id,
        "title": data.get('title', data.get('monitor_name', 'Datadog Alert')),
        "severity": alert_type,
        "status": data.get('alert_status', data.get('status', 'triggered')),
        "body": data.get('body', data.get('text', ''))[:500],
        "tags": data.get('tags', []),
        "url": data.get('url', data.get('monitor_url', '')),
        "host": data.get('hostname', data.get('host', '')),
        "timestamp": int(time.time()), "raw": data
    }
    producer.send('datadog-alerts', alert)
    producer.flush()
...
def grafana_webhook():
    ...
    if sub_alerts:
        for a in sub_alerts:
            ...
            alert = {
                "source": "grafana",
                "alert_id": a.get('fingerprint', a.get('id', 'unknown')),
                "title": labels.get('alertname', title),
                "severity": labels.get('severity', 'UNKNOWN'),
                "status": a.get('status', state),
                "summary": annots.get('summary', message),
                "description": annots.get('description', '')[:300],
                "labels": labels,
                "generator_url": a.get('generatorURL', ''),
                "timestamp": int(time.time()), "raw": a
            }
        ...
        alert = {
            "source": "grafana", "alert_id": data.get('groupKey', 'unknown'),
            "title": title, "severity": "UNKNOWN", "status": state,
            "summary": message, "description": message,
...
if __name__ == '__main__':
    print(f"[Producer] Flask starting on port {FLASK_PORT} ...")
